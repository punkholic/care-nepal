    ...
    return (
      ...
      >
        {markers.map((item) => {
          return(
            <Marker
            image={require('../../assets/foodLocation.png')}
            key={item.id} coordinate={{latitude: item.latitude, longitude: item.longitude}} title={item.name} description={item.place} 
            >
