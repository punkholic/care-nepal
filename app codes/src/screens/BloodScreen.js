...
const BloodScreen = ({navigation}) => {
    const [bloodData, updateBloodData]= useState([]);
    const [bloodType, updateBloodType]= useState({});
    const [location, updateLocation]= useState('');
    const blood_data= ["A-","A+","B-","B+", "AB+", "AB-", "O+", "O-"];
    useEffect(() => {
        selfMade.get('/blood').then((e) => updateBloodData(e.data)).catch(e => console.log(e))
    }, []);
    return (
        <>
        ...
        <ScrollView>
            ...
            <View style={styles.frontgroundView}>
                <Text style={styles.frontgroundViewText}>Find a Blood Donor</Text>
                ...
                onChange={(e) => {
                  updateBloodType(e)
                } } toShow="Select Blood Group"/>
                <Space marginBottom={20}/>
                <Button onClick={() => {
                        selfMade.get('/blood').then((e) => {
                            console.log(e.data)
                            let myData = e.data.filter((e) => {
                                return blood_data[e.blood_group] == bloodType.label && e.location == location
                            })
                            updateBloodData(myData)
                        }).catch(e => console.log(e))
                }} text="Search" color="red" size={20}/>
            </View>
            <View style={styles.recentDonarView}>
                <Text style={styles.recentDonarText}>Recent Donars</Text>
                <View style={styles.donarContact}>
                {bloodData.length !== 0 ? bloodData.map((e) => {
                        return <>
                        <Space marginBottom={20}/>
                        <Donar phone={`${e.mobile_number}`} location={`${e.location}`} name={`${e.firstname} ${e.lastname}`} bloodGroup={blood_data[e.blood_group]} color="red"/>
                        </>
